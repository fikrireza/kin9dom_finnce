...
@section('content')
	...
	<h1>Article Management - Create</h1>

	<form class="form-horizontal form-label-left" action="{{ route('admin.article.store') }}" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			<label for="id_article_category" class="control-label col-md-3 col-sm-3 col-xs-12">Category <span class="required">*</span></label>
			<div class="col-md-9 col-sm-9 col-xs-12">
				<select id="id_article_category" name="id_article_category" class="form-control {{$errors->first('id_article_category') != '' ? 'parsley-error' : ''}}">
					<option value="">-- Select Category --</option>
					@foreach($category as $list)
					<option value="{{ $list->id }}" @if(old('id_article_category') != '' && old('id_article_category') == $list->id) selected @endif>{{ $list->name }}</option>
					@endforeach
				</select>
				<ul class="parsley-errors-list filled">
					<li class="parsley-required">{{ $errors->first('id_article_category') }}</li>
				</ul>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<input type="file" id="image" name="image" class="form-control {{$errors->first('name') != '' ? 'parsley-error' : ''}}" value="{{ old('image') }}">
				<ul class="parsley-errors-list filled">
					<li class="parsley-required">{{ $errors->first('slug') }}</li>
				</ul>
			</div>
		...
		<div class="form-group">
			<div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
				{{ csrf_field() }}
				<a class="btn btn-primary" href="{{ route('admin.article') }}">Cancel</a>
