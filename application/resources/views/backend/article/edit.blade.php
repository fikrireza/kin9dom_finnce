...
@section('content')
	...
	<h1>Article Management - Edit</h1>
	<div class="x_panel">
		<form class="form-horizontal form-label-left" action="{{ route('admin.article.update', ['id' => $index->id]) }}" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
				...
				<div class="col-md-9 col-sm-9 col-xs-12">
					<select id="id_article_category" name="id_article_category" class="form-control {{$errors->first('id_article_category') != '' ? 'parsley-error' : ''}}">
						...
						<option value="{{ $list->id }}" @if($index->id_article_category == $list->id) selected @endif>{{ $list->name }}</option>
						@endforeach
					</select>
					<ul class="parsley-errors-list filled">
